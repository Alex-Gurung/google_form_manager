...
function pull_data() {
    ...
    for(var i = 0; i < tags.length; i++){  
        ...
        if (tags[i].type === 'text'){
            ...
            if (tags[i].value.length > 0){
                var lab = String(tags[i].getAttribute('aria-label'))
                // console.log(lab)
                values[lab] = tags[i].value
                }
            // if (tags[i].value.length > 0) {
            //     console.log(tags[i].value)
            // }
    ...
    console.log(values)
    window.localStorage.setItem(window.location.href, JSON.stringify(values));
    console.log('end')
    // console.log(tags)
}
function get_data() {
    var original = String(localStorage.getItem(window.location.href))
    var values = JSON.parse(localStorage.getItem(window.location.href))
    console.log(values)
    var tags = document.getElementsByTagName("input");
    // console.log('begin')
    for(var i = 0; i < tags.length; i++){  
        ...
        if (tags[i].type === 'text'){
            // console.log(tags[i].value)
            var lab = String(tags[i].getAttribute('aria-label'))
            if (values.hasOwnProperty(lab)){
                // console.log(lab)
                
                tags[i].value = values[lab]
                tags[i].parentElement.parentElement.parentElement.parentElement.className += ' hasValue'
                }
            // if (tags[i].value.length > 0) {
            //     console.log(tags[i].value)
            // }
    ...
    for(var i = 0; i < tags.length; i++){  
        // console.log(tags[i].type)
            ...
            if (values.hasOwnProperty(lab)){
                ...
                // console.log(lab)
                tags[i].value = values[lab]
                tags[i].parentElement.parentElement.parentElement.className += ' hasValue'
            }
            // if (tags[i].value.length > 0) {
            //     console.log(tags[i].value)
            // }
